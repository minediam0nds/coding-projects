...
Obviously the code isnt very efficient, and its quite messsy and unreadable (i never used comments), but all the ideas for the player's hand evaluation are mine and im pround of them :)
...
here's an explanation of how the code works so i wont subject you to the pain of reading through it:
    ...
    firstround(), secondround(), thirdround(), and  fourthround() are all basically the same, adding community cards according to poker rules and then allowing players to call, raise and fold. They all call the next function.
...
--------------------------------------------------------
    ...
    all of the functions also follow similar steps:
        ...
        4. pop and append the combination to returner in descending order to returner, if the card combination <5 cards (eg three of a kind) iterate backwards again and append until 5 cards.
    ...
    ill start from the simplest:
        ...
        straight():
            when sorting cards into valuelists, if the current number is (14), append 1 to valuelists[1], this is for a 'wheel straight', which is Ace, 2 , 3 , 4, 5.
            iterates backwards through valuelists until valuelist[i], valuelists[i-1]...,valuelists[i-4] have atleast one card, append and clear the 5 lists
   ...
   now for cardscheck():
        ...
        comparecards() first compares the first number of all the playerxvalues (starting at i=1 to skip the first as that is the player number) and adds the players who arent equal to the highest num to playersnotincompare
        player numbers inside playersnotincompare will be skipped in the comparing process and will be removed at the end
        if there is still >1 players left, it will repeat the for the next numbers until it reaches the end
        ...
        compare all playervalues[i=1]:
            player one is removed as his number is 2 which is less that 4
        compare all playervalues[i=2 to 5]:
            no change as all numbers are the same
        compare all playervalues[i=6]:
            playertwo is removed as it is less that 12
...
-------------------------------
    ...
    the bots would take in some factors of the game (like the amount it has to call and the value of its current hand) and add that to a number which will serve as a probability.
    the bot then takes a random integer and compares it to the probability value. if randint > probabilityvalue, call a players bet/make a bet , else fold or dont bet anything

    i chose this probability system so that the players will be unable to tell what the bot has in its hand, (e.g with only mathematical calculations choosing the amount to bet, like handvalue*50, the player can get a good idea on what the bot has when looking at the amont it bets.)
    ...
    the playerraised bool (only used in singleplayer) just represents whether the current bet raise was made by the player, and if it is True the callwillingness (probability value) will include playerbluffrate 
    ...
    playerbluffrate*100      -- this is the variable that i said the bots adapt to how the player plays
    ...
    so if a player bets a disproportionate amount of money e.g. 600 hundred when the player only has a pair , and the player has played three rounds before, playerbluffrate is calculated to 0.33 
    this means that when player raises in the next round, 0.33*100 = 33 will be added to the bots call probability. this is how the bot adapts to how the player plays, and becomes more likely to call out a bluff on a player who bluffs very often	
